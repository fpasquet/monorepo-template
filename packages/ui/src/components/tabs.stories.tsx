  ...
  play: async ({ canvas, step }) => {
    ...
    for (let i = 0; i < tabs.length; i++) {
      await step(`click the '${tabs[i]!.innerText}' tab`, async () => {
        ...
        await expect(canvas.queryByRole('tabpanel', { name: tabs[i]!.innerText })).toBeVisible();
      });

      await step('check other tabs are not selected', () => {
        for (let j = 0; j < tabs.length; j++) {
          if (j !== i) {
            expect(tabs[j]).toHaveAttribute('aria-selected', 'false');
            expect(canvas.queryByRole('tabpanel', { name: tabs[j]!.innerText })).toBeNull();
          }
        }
      });
    }
